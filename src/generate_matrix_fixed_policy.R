...
Obj[index_Q,] <- array(nT/12*costQ_rmt[,,1:12],dim=c(dim_Q,1))
Obj[index_F,] <- array(costF_rts,dim=c(dim_F,1))
Obj[index_2F,] <- array(0*costF_rts,dim=c(dim_2F,1))
Obj[index_IMP,] <- array(nT/12*costIMP_jmt[,,1:12],dim=c(dim_IMP,1))

#__________________________________________________________________________________________________
## GENERATE CONSTRAINTS MATRIX
#-- DEMANDS
rhs_d <- array(D_mt[,1:12],dim=c(nM*12,1))
s_d <- array(">=",dim=c(nM*12,1))
A_d <- array(0,dim=c(nM*12,dim_tot))
for(t in 1:12){for(m in 1:nM){
  for(r in 1:nR){
...
if(with_Pconstraints){
  # MIN
  rhs_pmin <- array(PCmin_rt[,1:12],dim=c(dim_releases,1))
  ...
  # MAX
  rhs_pmax <- array(PCmax_rt[,1:12],dim=c(dim_releases,1))
  s_pmax <- array("<",dim=c(dim_releases,1))
  A_pmax <- array(0,dim=c(dim_releases,dim_tot))
  for(l in 1:(nR*12)){
...
#-- IMPORTS
if(nIMP > 0){
  rhs_impmax <- array(IMPmax_jt[,1:12],dim=c(nIMP*12,1))
  s_impmax <- array("<",dim=c(nIMP*12,1))
  A_impmax <- array(0,dim=c(nIMP*12,dim_tot))
  for(j in 1:nIMP){for(t in 1:12){for(m in 1:nM){
...
for(r in 1:nR){
  for(s in 1:nS){
    ...
    for(t in 2:nT){
      for(m in 1:nM){
        A_sQ[posRTS(r,t,s),posRMT(r,m,((t-1)%%12+1))] <-Mrm[r,m]
      }
      A_sF[posRTS(r,t,s),posRTS(r,t,s)] <- -1
      A_s2F[posRTS(r,t,s),posRTS(r,t,s)] <- 1
      for(r_ in 1:nR){
        A_sP[posRTS(r,t,s),posRT(r_,((t-1)%%12+1))] <- (r==r_)-Mrr[r,r_]
        for(t_ in 1:nT){

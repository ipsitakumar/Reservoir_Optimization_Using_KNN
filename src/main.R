...
plotresults(rQ_PI, rQIMP_PI, rF_PI)
...
metricnoSF <- t(signif(rbind(Coef_PF, Coef_FP, Coef_PI, Coef_KNN),
       digits = 2))

volnoSF <- rbind(volnoSF, metricnoSF)
colnames(volnoSF) <- c("PF", "FP", "PI", "KNN")
...
for(rr in 1:nR){
  barplot(rbind(turntoyr(rQ_PF[rr,]), turntoyr(rQ_FP[rr,]), turntoyr(rQ_KNN[rr,]), turntoyr(rQ_PI[rr,])), beside = TRUE, col = c("blue", "red", "orange", "purple"))
}
...
for(rr in 1:nR){
  ...
  hist(turntoyr(rQ_PI[rr,]), col = "purple", breaks=br)
}
...
colnames(volwSF) <- c("PF", "FP", "PI", "KNN")
